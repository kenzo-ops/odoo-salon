...
class SalonBooking(models.Model):
    ...
    customer_number = fields.Char(
        related="customer.phone", string="Phone Number", readonly=True
    )
    customer_address = fields.Char(
        related="customer.street", string="Adress", readonly=True
    )
    customer_email = fields.Char(
        related="customer.email", string="Email", readonly=True
    )
    booking_date = fields.Datetime(string="Booking Schedule", required=True)
    end_date = fields.Datetime(
        string="Finish Time", compute="_compute_end_date", store=True, readonly=True
    )
    ...
    total_price = fields.Float(
        string="Total Price", compute="_compute_total_price", store=True, readonly=True
    )
    ...
    service_booking_id = fields.One2many(
        "salon.booking.service", "booking_id", string="Services"
    )
    package_booking_id = fields.One2many(
        "salon.booking.package", "booking_id", string="Packages"
    )

    working_hours = fields.Many2one(
        related="branch_id.working_hours_id", string="Working Hours"
    )
            ...
            if booking_hour < wh.working_start or booking_hour > wh.working_end:
                ...
                return {
                    "warning": {
                        ...
                        "message": (
                            f"Booking must be between {int(wh.working_start):02d}:"
    ...
    @api.depends(
        "booking_date",
        "service_booking_id.service_duration",
        "package_booking_id.package_duration",
    )
        ...
        for rec in self:
            if rec.booking_date:
                ...
                if rec.package_booking_id:
                    total_duration += sum(
                        pkg.package_duration or 0 for pkg in rec.package_booking_id
                    )

                rec.end_date = (
                    rec.booking_date + timedelta(minutes=total_duration)
                    if total_duration > 0
                    else False
                )
            else:
    ...
    @api.depends(
        "service_booking_id.service_price", "package_booking_id.package_total_price"
    )
    def _compute_total_price(self):
        for rec in self:
            total_services = sum(
                service.service_price or 0 for service in rec.service_booking_id
            )
            total_packages = sum(
                pkg.package_total_price or 0 for pkg in rec.package_booking_id
            )
            ...
            for package in rec.package_booking_id:
                invoice_lines.append(
                    (
                        ...
                        0,
                        {
                            "name": package.package_id.name or "Package",
                            "quantity": 1,
    ...
    def create(self, vals):
        ...
        if vals.get("booking_id", "New") == "New":
            vals["booking_id"] = (
                self.env["ir.sequence"].next_by_code("salon.booking") or "New"
            )
    ...
    invoice_ids = fields.One2many(
        "account.move", "booking_id", string="Invoices", readonly=True
    )
    invoice_count = fields.Integer(
        string="Total Invoices", compute="_compute_invoice_count", store=False
    )
